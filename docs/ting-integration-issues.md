...
Production rollout completed on September 23. Ting revision 3 is effective, and Hook's approved Ting scopes are active, including `receivers.bootstrap`. Hook 0.8.0, both migrated databases, the browser gateway, website, docs and all six CLI platforms are live. A dedicated `tos` publisher and the Hook notification type are configured. The [production verification record](verification/ting-live-0.8.0.md) covers real signed ingress, Ting delivery, exact hydration and acknowledgements. Other organizations still require their own publisher, and recipient automation opt-in remains explicit.
