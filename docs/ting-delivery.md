...
In IAM, Hook needs the Ting external scopes for `subscriptions.register`, `tings.send` and `sent.query`, plus `receivers.bootstrap` for scoped sandbox observation, with the required review and consent. Provision the `hook.webhook.received` Ting type with the authorized operator account. Type registration is a separate operator prerequisite; a missing type is not successful delivery. Local support does not activate pending Honeycomb scope revisions.
