...
The saved deployment-verification IAM refresh token was already consumed (HTTP 409), so this rollout does not claim a fresh authenticated end-to-end provider delivery. Automated IAM, isolation, WebSocket and delivery regression checks passed before publication.
